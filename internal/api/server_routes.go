...
func (s *Server) setupRoutes() {
	...
	v1.Use(AuthMiddleware(s.accessManager), s.keyPolicyMiddleware())
	{
	...
	s.engine.GET("/v1/realtime", realtimeAuth, s.keyPolicyMiddleware(), s.codexLiveHandler.HandleRealtimeWebsocket)
	s.engine.POST("/v1/realtime", realtimeAuth, s.keyPolicyMiddleware(), s.codexLiveHandler.Handle)
	s.engine.POST("/v1/realtime/calls", realtimeAuth, s.keyPolicyMiddleware(), s.codexLiveHandler.Handle)
	s.engine.GET("/v1/realtime/calls/:call_id", realtimeAuth, s.keyPolicyMiddleware(), s.codexLiveHandler.HandleSideband)
	s.engine.POST("/v1/realtime/client_secrets", standardAuth, s.keyPolicyMiddleware(), s.codexLiveHandler.CreateClientSecret)
	s.engine.POST("/v1/realtime/sessions", standardAuth, s.keyPolicyMiddleware(), s.codexLiveHandler.CreateLegacySession)
	s.engine.POST("/v1/realtime/transcription_sessions", standardAuth, s.keyPolicyMiddleware(), s.codexLiveHandler.HandleTranscriptionSession)
	s.engine.GET("/v1/realtime/translations", realtimeAuth, s.keyPolicyMiddleware(), s.codexLiveHandler.HandleTranslation)
	s.engine.POST("/v1/realtime/translations", realtimeAuth, s.keyPolicyMiddleware(), s.codexLiveHandler.HandleTranslation)
	s.engine.POST("/v1/realtime/translations/client_secrets", standardAuth, s.keyPolicyMiddleware(), s.codexLiveHandler.HandleTranslation)
	s.engine.POST("/v1/realtime/calls/:call_id/hangup", standardAuth, s.keyPolicyMiddleware(), s.codexLiveHandler.HandleHangup)
	s.engine.POST("/v1/realtime/calls/:call_id/accept", standardAuth, s.keyPolicyMiddleware(), s.codexLiveHandler.HandleSIPControl)
	s.engine.POST("/v1/realtime/calls/:call_id/reject", standardAuth, s.keyPolicyMiddleware(), s.codexLiveHandler.HandleSIPControl)
	s.engine.POST("/v1/realtime/calls/:call_id/refer", standardAuth, s.keyPolicyMiddleware(), s.codexLiveHandler.HandleSIPControl)

	openaiV1 := s.engine.Group("/openai/v1")
	openaiV1.Use(AuthMiddleware(s.accessManager), s.keyPolicyMiddleware())
	{
	...
	codexDirect.Use(AuthMiddleware(s.accessManager), s.keyPolicyMiddleware())
	{
	...
	v1beta.Use(AuthMiddleware(s.accessManager), s.keyPolicyMiddleware())
	{
...
func (s *Server) AttachWebsocketRoute(path string, handler http.Handler) {
	...
	conditionalAuth := func(c *gin.Context) {
		if !s.wsAuthEnabled.Load() && !s.keyPolicyConfigured {
			c.Next()
			return
		}
		authMiddleware(c)
	...
	s.engine.GET(trimmed, conditionalAuth, s.keyPolicyMiddleware(), finalHandler)
}
...
func (s *Server) handleGrokModels(c *gin.Context) {
	...
	if s != nil && s.cfg != nil && s.cfg.Home.Enabled {
		...
		if ids, scoped := s.handlers.AuthManager.KeyPolicyModelClients(c.Request.Context()); scoped {
			models = grokModelsFromRegistryInfos(registry.GetGlobalRegistry().GetModelInfosForClients(ids))
		} else {
